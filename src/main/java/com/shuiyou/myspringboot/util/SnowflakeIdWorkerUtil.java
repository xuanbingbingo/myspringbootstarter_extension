    ...
    public static SnowflakeIdWorkerUtil getInstance() {
        if (null == instance) {
            synchronized (SnowflakeIdWorkerUtil.class) {
                if (null == instance) {
                    // 根据ip地址的首端的第一个字符作为workerId，尾段最后一个字符做为datacenterId
                    String ip = IPAddressUtil.getLocalIpAddress();
                    String[] fds = StringUtils.split(ip, ".");
                    Random rand = new Random();
                    int workerId =
                            (null != fds && fds.length > 0)
                                    ? NumberUtils.toInt(
                                            StringUtils.left(fds[0], 1), rand.nextInt(30))
                                    : rand.nextInt(30);
                    int datacenterId =
                            (null != fds && fds.length > 0)
                                    ? NumberUtils.toInt(
                                            StringUtils.right(fds[fds.length - 1], 1),
                                            rand.nextInt(30))
                                    : rand.nextInt(30);
                    instance = new SnowflakeIdWorkerUtil(workerId, datacenterId);
                }
            }
        }
        return instance;
